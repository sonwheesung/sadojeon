...
- **흐름 (플랜 → 문서 → 개발 → 오류확인, AGENTS.md `# 작업 순서` 정본)**: ⓪ **플랜** — 무엇을·왜·어디를 건드릴지 먼저 정한다(모호하면 실물 확인, 그래도 모호하면 사용자에게) → ① 해당 docs(설계·계약·수치)에 무엇을 만들지 먼저 적는다(상호 참조·밴드·계약 포함) → ② 그 문서를 기준으로 코드 작성 → ③ docs/40 영향 계층 테스트로 코드가 문서대로인지 검증(새 콘텐츠면 docs/43 사냥 5렌즈). 문서와 코드가 어긋나면 **코드를 문서에 맞춘다**(설계 자체를 바꿀 거면 문서를 먼저 고친 뒤 코드).
