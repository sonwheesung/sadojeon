...
"전체 테스트 진행"은 **등록된 모든 테스트 케이스를 빠짐없이 실행하고, 각각의 결과를 실제로 확인·검증**한다는 뜻이다. 일부만 돌리거나, 돌렸다고 가정하거나, 통과를 추정으로 단정하지 않는다([feedback_grounded_genre_dev]).

1. **전 케이스 실행** — docs/40 §2의 전 계층(tsc·tsx loop·esbuild loop·`npm test`·factorysweep·testguard)을 하나도 건너뛰지 않고 돈다. 무거워 시간이 걸려도(extremerisk 등) 끝까지 완주시킨다 — 느린 건 생략 사유가 아니다.
2. **각 결과를 눈으로 확인** — 모든 sim의 끝줄(`결과: N PASS · 0 FAIL`·"전부 기대 범위 안"·"전 계약 PASS ✓")과 `npm test`의 `Tests: N passed`를 **실제 출력에서 확인**한 뒤 표에 옮긴다. "아마 통과했을 것"으로 채우지 않는다.
3. **검증 = 단순 무크래시가 아니다** — exit 0 이어도 단언(check/ck/band)이 실제로 PASS인지, 통계형은 밴드 안인지까지 본다. 밴드 밖이면 statcheck로 정량 → 의도/회귀 판정(§3).
4. **누락이 곧 사고** — 어떤 케이스가 실행 목록에서 빠졌다면(§1 감사로 검출), 그 자체가 실패다. 자가치유로 docs/40 §2에 넣고 보고에 명시한 뒤 다시 전수 실행.
