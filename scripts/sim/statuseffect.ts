...
// 체질(불침) — ① 이미 그 속성 상처를 안고 싸울 때 결 차단 ② 결정타가 그 속성을 못 입힘.
console.log('\n[체질 — 위중(sev2) 상처 안고 싸울 때, 면역이면 결 차단(승률↑)]');
row('중독·저항없음', '독 먹힘', { woundSeverity: 2, woundType: 'poison' });
row('중독·천독불침', '일반독 면역', { woundSeverity: 2, woundType: 'poison', woundResist: { poison: 1 } });
row('중독·만독불침', '완전 면역', { woundSeverity: 2, woundType: 'poison', woundResist: { poison: 2 } });
row('동상·한서불침', '동상 면역', { woundSeverity: 2, woundType: 'frost', woundResist: { frost: 2 } });
row('화상·화염불침', '화상 면역', { woundSeverity: 2, woundType: 'burn', woundResist: { burn: 2 } });
row('외상 sev2(비교)', '속성 아님', { woundSeverity: 2, woundType: 'wound' });

// 결정타가 방어자에게 실제로 그 속성 상처를 입히는가 — 체질별 입은 상처 종류 비율.
console.log('\n[체질 — 속성 결정타 맞은 방어자가 그 속성 상처를 입는 비율]');
function woundTypeRate(atkTrait: 'poison' | 'frost' | 'burn', defResist: Combatant['woundResist'], want: string): number {
  let downed = 0, hit = 0;
  for (let i = 0; i < N; i += 1) {
    const atk = make('atk', {
      realm: 'jeoljeong', internal: 600, strength: 55,
      arts: [{ school: 'hidden', grade: 'master', path: 'jung', seong: 7, isMain: true, traits: [atkTrait] }],
    });
    const r = simulateCombat([atk], [make('def', { woundResist: defResist })], { mode: 'real' });
    const def = r.combatants.find((c) => c.id === 'def');
    if (def && (def.state === 'downed' || def.state === 'dead') && def.wound) {
      downed += 1;
      if (def.wound.type === want) hit += 1;
    }
  }
  return downed ? (hit / downed) * 100 : 0;
}
for (const [label, trait, resist, want] of [
  ['독공 vs 저항없음', 'poison', {}, 'poison'],
  ['독공 vs 천독불침', 'poison', { poison: 1 }, 'poison'],
  ['독공 vs 만독불침', 'poison', { poison: 2 }, 'poison'],
  ['빙공 vs 한서불침', 'frost', { frost: 2 }, 'frost'],
] as const) {
  const pct = woundTypeRate(trait, resist, want);
  console.log(`  ${label.padEnd(20)} 쓰러진 방어자 중 ${want} 상처 ${pct.toFixed(1)}%`);
}
