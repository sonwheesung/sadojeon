...
export interface Combatant {
  ...
  // 기존 부상 종류 — 심도(공세 저하)는 공통, 종류가 추가 결을 정한다. docs/35 §6-1b.
  //  외상=공세만 / 동상=신법↓ / 중독·화상=합마다 지속 피해 / 내상=내공 빨리 마름.
  woundType?: WoundType;
  // 체질(불침) — 속성별 상처 저항. 0/없음 미수록 / 1 부분(천독불침) / 2 완전(금강불괴·한서·화염·만독불침).
  // 무공 수련 깊이로 결정(woundResistOf). docs/35 §6-1c.
  woundResist?: Partial<Record<WoundType, number>>;
}
