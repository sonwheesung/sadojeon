...
import { grantIntroRunReward } from './introRunReward';
...
export function endRun(): void {
  ...
  // 도입 튜토리얼 회차 끝맺음 보상 — 중품 비급 1권(계정 영속 → 2회차 훅) + 다이아. reset 전에 지급. docs/46.
  if (useRunMetaStore.getState().isTutorialRun) grantIntroRunReward();
  checkAchievements();
