...
import { defaultArtTraits, resistsWound } from '@/data/martialArts';
...
export function buildSheet(c: Combatant, foeMeanInternal: number): CombatSheet {
  ...
  // 체질(불침)이면 제 속성 상처의 결이 안 먹힌다(한서불침→동상 무효, 화염불침→화상 무효, 만독불침→중독 무효).
  const wd = woundDepth(c.woundSeverity);
  const immuneTo = (t: typeof c.woundType) =>
    t != null && c.woundType === t && resistsWound(c.woundResist?.[t], c.woundSeverity ?? 5);
  const frostSlow = c.woundType === 'frost' && !immuneTo('frost') ? 1 - 0.25 * wd : 1; // 동상: 신법 최대 -25%
  const innerDrainMult = c.woundType === 'inner' ? 1 + 0.6 * wd : 1; // 내상: 내공 소모 최대 1.6배(면역 체질 없음)
  const dotFrac =
    c.woundType === 'poison' && !immuneTo('poison')
      ? 0.015 * wd
      : c.woundType === 'burn' && !immuneTo('burn')
        ? 0.01 * wd
        : 0; // 합당 지속 피해
